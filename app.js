    ...
    methods: {
        drawShape: function () {
            ...
            if (this.width % 2 !== 0  || this.width < 20 || this.width > 300) {
                alert(`Height value should be even and greater or equal to 20 and less than or equal 300 ${this.width} given`)
                return;
            }
            const getShapeCordinates = this.makeShape(this.width, this.height , this.padding);
            const output = document.querySelector("pre");
            output.textContent = getShapeCordinates.map(row => row.map(i => " -|"[i]).join``).join`\n`;
